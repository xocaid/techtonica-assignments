...
const Users = () => {
  ...
  const handleAddNewUser = async (e) => {
    ...
    try {
      ...
      //if statement if response.ok, then print:
      if (response.ok) {
        setUsers([...users, content]);
        setNewUser({ first_name: "", last_name: "", email: "", phone_number: "" });
        setPhoneError(null);
        setEmailError(null);
      } else if (content.message === 'Phone Number Validation Failed.') {
        //content.message --> From `content` in handleAddNewUser, the message is provided from the backend 
      ...
      //else logic for frontend to do something with it
    } catch (error) {
      //catch is for if backend fails or is not running - specifically for the add new user
      console.error('Add New User: Backend is broken or not running')
    }
  };
            ...
            if (user.id === editingUserId) {
              ...
              return (
                <tr key={index}>
                  ...
                  <td><img src={trash} className='icon-btn' alt='trash-icon' onClick={() => handleDeleteUser(user.id)} /></td>
                  <td><img src={edit} className="icon-btn" alt='edit-icon' onClick={() => setEditingUserId(user.id)} /></td>
                </tr>
      ...
      <div className="add_Divs">
        ...
        <form className='addForm' id="add-user" action="#" onSubmit={handleAddNewUser}>
          <fieldset>
            ...
            />
            <span style={{ color: 'red', fontSize: '14px', fontStyle: 'italic' }}>{emailError}</span>
            <br />
            ...
            />
            <br />
            <span style={{ color: 'red', fontSize: '14px', fontStyle: 'italic' }}>{phoneError}</span>
          </fieldset>
